...
namespace libff {
	...
	size_t Double::s; // TODO: add by hyg
  ...
  {
    return Double(pow(val, (double)power.as_integer()));
  }
  ...
  {
    return (unsigned long)round(val.real());
  }

	size_t Double::as_integer() const
	{
		return (size_t)round(val.real());
	}
