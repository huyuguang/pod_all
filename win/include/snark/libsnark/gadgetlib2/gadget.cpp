...
R1P_AND_Gadget::R1P_AND_Gadget(ProtoboardPtr pb,
    ...
    GADGETLIB_ASSERT(input.size() <= Fp(-1).as_integer(), "Attempted to create R1P_AND_Gadget with too "
...
R1P_OR_Gadget::R1P_OR_Gadget(ProtoboardPtr pb,
    ...
    GADGETLIB_ASSERT(input.size() <= Fp(-1).as_integer(), "Attempted to create R1P_OR_Gadget with too "
                                                              "many inputs. Will cause overflow!");
...
R1P_LooseMUX_Gadget::R1P_LooseMUX_Gadget(ProtoboardPtr pb,
    ...
    GADGETLIB_ASSERT(inputs.size() <= Fp(-1).as_integer(), "Attempted to create R1P_LooseMUX_Gadget "
...
void R1P_LooseMUX_Gadget::generateWitness() {
    ...
    const size_t index = val(index_).asInteger();
...
void R1P_CompressionPacking_Gadget::generateWitness() {
    ...
    if (packingMode_ == PackingMode::PACK) {
        ...
        FElem two_i(R1P_Elem(1)); // will hold 2^i
        for(int i = 0; i < n; ++i) {
            GADGETLIB_ASSERT(val(unpacked_[i]).asInteger() == 0 || val(unpacked_[i]).asInteger() == 1,
                         GADGETLIB2_FMT("unpacked[%u]  = %u. Expected a Boolean value.", i,
                             val(unpacked_[i]).asInteger()));

            //packedVal += two_i * val(unpacked_[i]).asInteger();
						auto tmp = two_i;
						tmp *= val(unpacked_[i]).asInteger();
						packedVal += tmp;
