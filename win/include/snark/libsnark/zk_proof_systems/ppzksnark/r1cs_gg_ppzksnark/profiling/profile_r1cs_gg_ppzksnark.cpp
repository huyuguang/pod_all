...
int main(int argc, const char * argv[])
{
    ...
    size_t input_size = (size_t)atoi(argv[2]);
    if (argc == 4)
    {
        assert(strcmp(argv[3], "Fr") == 0 || strcmp(argv[3], "bytes") == 0);
        if (strcmp(argv[3], "bytes") == 0)
        {
            input_size = libff::div_ceil((size_t)8 * (size_t)input_size,
							libff::Fr<libff::default_ec_pp>::capacity());
        }
    }

    libff::enter_block("Generate R1CS example");
    r1cs_example<libff::Fr<default_r1cs_gg_ppzksnark_pp> > example = 
			generate_r1cs_example_with_field_input<libff::Fr<default_r1cs_gg_ppzksnark_pp> >(num_constraints, input_size);
