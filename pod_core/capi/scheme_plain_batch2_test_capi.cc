#include "scheme_plain_batch2_test_capi.h"
...
class WrapperSession {
 public:
  WrapperSession(handle_t c_a, uint8_t const* c_self_id,
    ...
    h_ = E_PlainBatch2SessionNew(c_a, c_self_id, c_peer_id);
    if (!h_) throw std::runtime_error("");
  }
  ~WrapperSession() {
    if (!E_PlainBatch2SessionFree(h_)) abort();
  }
  handle_t h() const { return h_; }
...
class WrapperClient {
 public:
  WrapperClient(handle_t c_b, uint8_t const* c_self_id,
    ...
    h_ = E_PlainBatch2ClientNew(c_b, c_self_id, c_peer_id, c_demand,
                                c_demand_count);
    if (!h_) throw std::runtime_error("");
  }
  ~WrapperClient() {
    if (!E_PlainBatch2ClientFree(h_)) abort();
  }
  handle_t h() const { return h_; }
...
namespace scheme::plain::batch2::capi {
...
bool Test(std::string const& output_path, WrapperA const& a, WrapperB const& b,
  ...
  if (evil) E_PlainBatch2SessionSetEvil(session.h());
  ...
  if (!E_PlainBatch2ClientGetRequest(client.h(), request_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_PlainBatch2SessionOnRequest(session.h(), request_file.c_str(),
                                     response_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_PlainBatch2ClientOnResponse(client.h(), response_file.c_str(),
                                     receipt_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_PlainBatch2SessionOnReceipt(session.h(), receipt_file.c_str(),
                                     secret_file.c_str())) {
    assert(false);
    return false;
  }
  ...
  if (!evil) {
    if (!E_PlainBatch2ClientOnSecret(client.h(), secret_file.c_str())) {
      assert(false);
      return false;
    }

    if (!E_PlainBatch2ClientSaveDecrypted(client.h(), output_file.c_str())) {
      assert(false);
      return false;
    }
  } else {
