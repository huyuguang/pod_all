#include "scheme_table_atomic_swap_vc_test_capi.h"
...
class WrapperAlice {
 public:
  WrapperAlice(handle_t c_alice_data, uint8_t const* c_self_id,
    ...
    h_ = E_TableAtomicSwapVcAliceNew(c_alice_data, c_self_id, c_peer_id);
    if (!h_) throw std::runtime_error("");
  }
  ~WrapperAlice() {
    if (!E_TableAtomicSwapVcAliceFree(h_)) abort();
  }
  handle_t h() const { return h_; }
...
class WrapperBob {
 public:
  WrapperBob(handle_t c_bob_data, uint8_t const* c_self_id,
    ...
    h_ = E_TableAtomicSwapVcBobNew(c_bob_data, c_self_id, c_peer_id, c_demand,
                                 c_demand_count);
    if (!h_) throw std::runtime_error("");
  }
  ~WrapperBob() {
    if (!E_TableAtomicSwapVcBobFree(h_)) abort();
  }
  handle_t h() const { return h_; }
...
bool Test(std::string const& output_path, WrapperAliceData const& a,
  ...
  if (evil) E_TableAtomicSwapVcAliceSetEvil(alice.h());
  ...
  if (!E_TableAtomicSwapVcBobGetRequest(bob.h(), request_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_TableAtomicSwapVcAliceOnRequest(alice.h(), request_file.c_str(),
                                       response_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_TableAtomicSwapVcBobOnResponse(bob.h(), response_file.c_str(),
                                      receipt_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_TableAtomicSwapVcAliceOnReceipt(alice.h(), receipt_file.c_str(),
                                       secret_file.c_str())) {
    assert(false);
    return false;
  }
  ...
  if (!evil) {
    if (!E_TableAtomicSwapVcBobOnSecret(bob.h(), secret_file.c_str())) {
      assert(false);
      return false;
    }

    if (!E_TableAtomicSwapVcBobSaveDecrypted(bob.h(), output_file.c_str())) {
      assert(false);
      return false;
    }
  } else {
