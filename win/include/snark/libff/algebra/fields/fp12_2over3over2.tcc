...
template<mp_size_t n, const bigint<n>& modulus>
Fp12_2over3over2_model<n,modulus> Fp12_2over3over2_model<n,modulus>::Frobenius_map(size_t power) const
{
...
Fp12_2over3over2_model<n, modulus> Fp12_2over3over2_model<n,modulus>::cyclotomic_exp(const bigint<m> &exponent) const
{
    ...
    {
        for (ssize_t j = GMP_NUMB_BITS - 1; j >= 0; --j)
        {
            if (found_one)
            {
