                                                       ...
                                                       const ram_input_tape<ramT> &auxiliary_input)
{
    ...
    {
        /* load instruction */
        const size_t pc_addr = this->pb.val(load_instruction_lines[i].address->packed).as_integer();
        ...
        /* fill it in */
        const size_t load_store_addr = this->pb.val(execution_lines[i+1].address->packed).as_integer();
        ...
        const size_t load_store_next_val = this->pb.val(execution_lines[i+1].contents_after->packed).as_integer();
    ...
    */
    ...
    {
        mem_pairs.emplace_back(std::make_pair(this->pb.val(unrouted_memory_lines[i]->address->packed).as_integer(),
                                              this->pb.val(unrouted_memory_lines[i]->timestamp->packed).as_integer()));
    }
    ...
    for (size_t i = 0; i < this->num_memory_lines; ++i)
    {
        const size_t timestamp = this->pb.val(unrouted_memory_lines[i]->timestamp->packed).as_integer();
        const size_t address = this->pb.val(unrouted_memory_lines[i]->address->packed).as_integer();
...
void ram_universal_gadget<ramT>::print_execution_trace() const
{
    for (size_t i = 0; i < boot_trace_size_bound; ++i)
    {
        printf("Boot process at t=#%zu: store %zu at %zu\n",
               i,
               this->pb.val(boot_lines[i].contents_after->packed).as_integer(),
               this->pb.val(boot_lines[i].address->packed).as_integer());
    }
    ...
    for (size_t i = 0; i < time_bound; ++i)
    {
        ...
        printf("  Loaded instruction %zu from address %zu (ts = %zu)\n",
               this->pb.val(load_instruction_lines[i].contents_after->packed).as_integer(),
               this->pb.val(load_instruction_lines[i].address->packed).as_integer(),
               this->pb.val(load_instruction_lines[i].timestamp->packed).as_integer());
        ...
        printf("  Memory operation executed: addr = %zu, contents_before = %zu, contents_after = %zu (ts_before = %zu, ts_after = %zu)\n",
               this->pb.val(execution_lines[i+1].address->packed).as_integer(),
               this->pb.val(execution_lines[i+1].contents_before->packed).as_integer(),
               this->pb.val(execution_lines[i+1].contents_after->packed).as_integer(),
               this->pb.val(execution_lines[i].timestamp->packed).as_integer(),
               this->pb.val(execution_lines[i+1].timestamp->packed).as_integer());
    }
}
...
void ram_universal_gadget<ramT>::print_memory_trace() const
{
    for (size_t i = 0; i < num_memory_lines; ++i)
    {
        ...
        printf("  Time side  : ts = %zu, address = %zu, contents_before = %zu, contents_after = %zu\n",
               this->pb.val(unrouted_memory_lines[i]->timestamp->packed).as_integer(),
               this->pb.val(unrouted_memory_lines[i]->address->packed).as_integer(),
               this->pb.val(unrouted_memory_lines[i]->contents_before->packed).as_integer(),
               this->pb.val(unrouted_memory_lines[i]->contents_after->packed).as_integer());
        printf("  Memory side: ts = %zu, address = %zu, contents_before = %zu, contents_after = %zu\n",
               this->pb.val(routed_memory_lines[i].timestamp->packed).as_integer(),
               this->pb.val(routed_memory_lines[i].address->packed).as_integer(),
               this->pb.val(routed_memory_lines[i].contents_before->packed).as_integer(),
               this->pb.val(routed_memory_lines[i].contents_after->packed).as_integer());
    }
}
