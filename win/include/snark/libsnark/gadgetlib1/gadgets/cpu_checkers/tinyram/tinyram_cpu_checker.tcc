...
void tinyram_cpu_checker<FieldT>::generate_r1cs_witness_address()
{
    ...
    ls_addr.fill_with_bits_of_field_element(this->pb, this->pb.val(arg2val->packed).as_integer() >> this->pb.ap.subaddr_len());
}
                                                              ...
                                                              const tinyram_input_tape_iterator &aux_end)
{
    ...
    const size_t opcode_val = opcode.get_field_element_from_bits(this->pb).as_integer();
    for (size_t i = 0; i < ((size_t)1)<<this->pb.ap.opcode_width(); ++i)
    {
    ...
    const size_t prev_doubleword = this->pb.val(ls_prev_val_as_doubleword_variable->packed).as_integer();
    const size_t subaddress = this->pb.val(memory_subaddress->packed).as_integer();
    ...
    {
        const size_t stored_byte = (this->pb.val(desval->packed).as_integer()) & 0xFF;
        this->pb.val(memory_subcontents) = FieldT(stored_byte);
    }
    else if (this->pb.val(opcode_indicators[tinyram_opcode_STOREW]) == FieldT::one())
    {
        const size_t stored_word = (this->pb.val(desval->packed).as_integer());
...
void tinyram_cpu_checker<FieldT>::dump() const
{
    printf("   pc = %zu, flag = %zu\n",
           this->pb.val(prev_pc_addr_as_word_variable->packed).as_integer(),
           this->pb.val(prev_flag).as_integer());
    printf("   ");

    for (size_t j = 0; j < this->pb.ap.k; ++j)
    {
        printf("r%zu = %2zu ", j, this->pb.val(prev_registers[j].packed).as_integer());
    }
    printf("\n");
    ...
    printf("   %s r%zu, r%zu, %s%zu\n",
           ...
           desidx.get_field_element_from_bits(this->pb).as_integer(),
           arg1idx.get_field_element_from_bits(this->pb).as_integer(),
           (this->pb.val(arg2_is_imm) == FieldT::one() ? "" : "r"),
           arg2idx.get_field_element_from_bits(this->pb).as_integer());
}
