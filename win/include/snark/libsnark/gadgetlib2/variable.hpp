...
public:
    ...
    virtual mp_limb_t asInteger() const = 0;
...
public:
    ...
    FElem inverse(const FieldType& fieldType);
		mp_limb_t asInteger() const {return elem_->asInteger();}
...
public:
    ...
    virtual FElemInterfacePtr inverse() const;
		mp_limb_t asInteger() const {return contents_;}
...
public:
    ...
    virtual R1P_Elem& operator=(const FConst& src) {elem_ = src.asInteger(); return *this;}
    virtual R1P_Elem& operator=(const mp_limb_t n) {elem_ = Fp(n); return *this;}
    virtual ::std::string asString() const {return GADGETLIB2_FMT("%ull", (uint64_t)elem_.as_integer());}
    ...
    virtual bool operator==(const FConst& other) const {return elem_ == Fp(other.asInteger());}
