...
public:
    ...
    Fp_model(const mp_limb_t x, const bool is_unsigned=false);

    void set_integer(const mp_limb_t x);
    ...
    Fp_model& operator-=(const Fp_model& other);
    Fp_model& operator*=(const Fp_model& other);
    Fp_model& operator^=(const mp_limb_t pow);
    ...
    Fp_model operator^(const mp_limb_t pow) const;
    template<mp_size_t m>
