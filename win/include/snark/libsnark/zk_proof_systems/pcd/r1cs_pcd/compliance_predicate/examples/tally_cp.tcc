...
public:
    ...
    std::shared_ptr<r1cs_pcd_message<FieldT> > get_message() const
    {
        ...
        const size_t sum_val = sum_bits.get_field_element_from_bits(this->pb).as_integer();
        const size_t count_val = count_bits.get_field_element_from_bits(this->pb).as_integer();
...
public:
    ...
    {
        const size_t summand_val = this->pb.val(summand).as_integer();
