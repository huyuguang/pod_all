        ...
        */
        ...
        if (has_digit) {
            ...
            if (prob_float)
                ...
                if (!neg_allowed) log10_num_buff = log10l(-num_buff);
                else log10_num_buff = log10l(num_buff);

                if (log10_num_buff < log10l(std::numeric_limits<int>::max()))
                    return CSV_INT;
                else if (log10_num_buff < log10l(std::numeric_limits<long int>::max()))
                    return CSV_LONG_INT;
                else if (log10_num_buff < log10l(static_cast<long double>(std::numeric_limits<long long int>::max())))
