...
template<mp_size_t n, const bigint<n>& modulus>
Fp_model<n,modulus>::Fp_model(const mp_limb_t x, const bool is_unsigned)
{
    static_assert(std::numeric_limits<mp_limb_t>::max() >=
			static_cast<unsigned long>(std::numeric_limits<long>::max()),
			"long won't fit in mp_limb_t");
    if (is_unsigned || x >= 0)
    {
...
template<mp_size_t n, const bigint<n>& modulus>
void Fp_model<n,modulus>::set_integer(const mp_limb_t x)
{
...
template<mp_size_t n, const bigint<n>& modulus>
mp_limb_t Fp_model<n,modulus>::as_integer() const
{
    return this->as_bigint().as_integer();
}
