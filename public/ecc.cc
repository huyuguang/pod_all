...
struct omembuf : public std::streambuf {
  omembuf(char* array, size_t len) {
    this->setp(array, array + len);
  }
};
...
Fr BinToFr31(void const* start, void const* end) {
  ...
  Fr fr;
  imemstream in((char*)buf, 32);
  fr.load(in, mcl::IoMode::IoSerialize);  // never throw
  return fr;
}
...
bool BinToFr32(void const* start, Fr* fr) {
  bool ret;
  imemstream in((char*)start, 32);
  fr->load(&ret, in, mcl::IoMode::IoSerialize);
  return ret;
}
...
void G1ToBin(G1 const& g, uint8_t* buf) {
  omemstream out((char*)buf, 32);
  g.save(out, mcl::IoMode::IoSerialize);
}
